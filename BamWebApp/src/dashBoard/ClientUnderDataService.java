...
public class ClientUnderDataService {
	...
	public static List<Client> getClientListUnder() {
		...
		try {
			...
			String query = "SELECT p.client_name,p.type,p.activity,p.timestamp,p.Final_status FROM process_state_table p where p.Final_status='' UNION SELECT m.client_name,m.type,m.Activity,m.DateTimeStamp,m.Final_status FROM mj_report m where m.Final_status=''";

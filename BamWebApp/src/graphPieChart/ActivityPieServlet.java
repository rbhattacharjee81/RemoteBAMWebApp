...
@WebServlet("/activityPieServlet")
public class ActivityPieServlet extends HttpServlet {

	private static final long serialVersionUID = 1L;

	public ActivityPieServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request,
		...
		int graphPlotPie = populateActivityPieVisits();
		int populateFailureListPieChart = populateFailurePieChart();
		int populateUnderProcessListPieChart = populateUnderProcessPieChart();

		String graphPlotPieString = String.valueOf(graphPlotPie);
		String stringpopulateFailureListPieChart = String
				.valueOf(populateFailureListPieChart);
		String tringpopulateUnderProcessListPieChart = String
				.valueOf(populateUnderProcessListPieChart);
...
//				tringpopulateUnderProcessListPieChart };
	...
	private int populateActivityPieVisits() {
		...
		try {
			...
			String query = "SELECT (SELECT COUNT(distinct timestamp) FROM process_state_table WHERE  Final_status = 'Processed')+(SELECT COUNT(distinct DateTimeStamp) from mj_report WHERE Final_status = 'Processed') AS SumCount";
	...
	private int populateFailurePieChart() {
		...
		try {
			...
			Connection connection = (Connection) dbConnection.dbConnection();
			st = connection.createStatement();
			String query = "SELECT (SELECT COUNT(distinct timestamp) FROM process_state_table WHERE  Final_status = 'Failure')+(SELECT COUNT(distinct DateTimeStamp) from mj_report WHERE Final_status = 'Failure') AS SumCount";
			rs = st.executeQuery(query);
	...
	private int populateUnderProcessPieChart() {
		...
		try {
			...
			Connection connection = (Connection) dbConnection.dbConnection();
			st = connection.createStatement();
			String query = "SELECT (SELECT COUNT(distinct timestamp) FROM process_state_table WHERE  Final_status = '')+(SELECT COUNT(distinct DateTimeStamp) from mj_report WHERE Final_status = '') AS SumCount";
			rs = st.executeQuery(query);
